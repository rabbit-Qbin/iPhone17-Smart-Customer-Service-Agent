...
class MessagesState(TypedDict):
    ...
    # 定义relevance_score字段，用于存储文档相关性判断结果
    relevance_score: Annotated[Optional[str], "Relevance check result of retrieved documents, 'yes' or 'no'"]
    ...
    def _build_routing_config(self, tools):
        ...
        for tool in tools:
            ...
            if "retrieve" in tool_name:
                # 如果是检索类工具，将其路由目标设置为 "grade_documents"（需要相关性判断）
...
class DocumentRelevanceScore(BaseModel):
    # 定义binary_score字段，表示相关性判断结果，取值为"yes"或"no"
...
# 定义Node grade_documents相关性判断函数
def grade_documents(state: MessagesState, llm_chat) -> dict:
    """判断检索到的文档内容与问题是否相关，并将判断结果存储在状态中。
    ...
    if not state.get("messages"):
        ...
        return {
            "messages": [{"role": "system", "content": "状态为空，无法判断"}],
    ...
    try:
        ...
        logger.info(f"Document relevance result: {score}")
        ...
        return {
            "messages": [{"role": "system", "content": "无法判断文档相关性"}],
            "relevance_score": None
        }
    except Exception as e:
        logger.error(f"Unexpected error in grading: {e}")
        return {
            "messages": [{"role": "system", "content": "判断过程中出错"}],
            "relevance_score": None
        }
...
def create_graph(db_connection_pool: ConnectionPool, llm_chat, llm_embedding, tool_config: ToolConfig) -> StateGraph:
    ...
    try:
        ...
        embed_dims = 768 if Config.LLM_TYPE in ["deepseek", "ollama"] else 1536
        store = PostgresStore(db_connection_pool, index={"dims": embed_dims, "embed": llm_embedding})
        store.setup()
    except Exception as e:
